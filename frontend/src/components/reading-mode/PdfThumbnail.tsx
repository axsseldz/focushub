...
export function PdfThumbnail({
  ...
  return (
    <div className="relative aspect-3/4 overflow-hidden rounded-[1.35rem] border border-slate-200/80 bg-[linear-gradient(180deg,#f8fafc,#eef2f7)] p-2">
      {isLoading ? (
        <div className="absolute inset-2 animate-pulse rounded-2xl bg-[linear-gradient(180deg,#f8fafc,#eef2f7)]" />
      ) : null}
      ...
      {!isLoading && !previewSrc ? (
        <div className="absolute inset-2 flex items-center justify-center rounded-2xl bg-slate-100 text-sm font-medium text-slate-400">
          PDF
        </div>
      ) : null}
    </div>
