...
export function GestureCamera({ enabled, onNextPage, onPrevPage }: GestureCameraProps) {
  ...
  return (
    <div
      ...
      className="fixed bottom-6 right-6 z-50 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_18px_48px_rgba(15,23,42,0.12)] dark:border-zinc-700 dark:bg-zinc-900"
    >
      {/* Loading */}
      {status === "loading" && (
        <div className="flex h-32 w-44 flex-col items-center justify-center gap-2 bg-slate-50 dark:bg-zinc-800">
      ...
      {status === "active" && videoHidden && (
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <span className="text-[11px] font-medium text-slate-600 dark:text-zinc-300">Gestos activos</span>
          <button
            type="button"
            onClick={() => setVideoHidden(false)}
            aria-label="Mostrar cámara"
            className="ml-1 rounded-full p-0.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:text-zinc-500 dark:hover:bg-zinc-700 dark:hover:text-zinc-200"
          >
            <EyeIcon />
          </button>
        </div>
